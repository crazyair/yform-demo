...
const handleImport = (file: string) => {
	...
	const finalContent = (file || '').replace(importRegex, matchedImport => {
		if (matchedImport.includes('.')) {
			...
			if (matchedImport.includes(',')) {
				const replaceLib = matchedImport.replace(/['|"].*['|"]/, "'yform-demo'");
			...
			// relative import;
			if (matchedImport.includes('{')) {
				return matchedImport.replace(/['|"].*['|"]/, "'yform-demo'");
			}
...
/** This is an auto-generated demo from yform-demo
					...
					<div style={{ float: 'right', marginTop: -16 }}>
						...
						<span style={{ marginRight: 16 }}>
							<Tooltip title={isChinese ? '全屏查看' : 'fullscreen'}>
								<FullscreenOutlined onClick={() => setShowModel(v => !v)} />
							</Tooltip>
						</span>
						<form
